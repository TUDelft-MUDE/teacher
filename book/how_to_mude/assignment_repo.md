...
4. Add a empty repository (if you already have a template repository, make it based on that template) and give it a logical name. In MUDE we use:
...
7. Go to {octicon}`repo-push` `Rules` - `Rulesets`, click `Import a ruleset` and import [this file](./Protect%20main.json). This will require pull requests for the main branch with 1 approval, allowing bypass by organization admins
