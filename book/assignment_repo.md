...
To do this, all of the assignment repositories include a template notebook, template readme and template report (as included [here](https://github.com/TUDelft-MUDE/assignment_repo_template).

### Assignment and solution version
An assignment and solution version is created for all the markdown and notebook files when pushing to the main branch (using [the workflow file included in repo_template](https://github.com/TUDelft-MUDE/assignment_repo_template)). Therefore, the main branch should include all information.

Based on the cell tags `solution` and `assignment` the notebook are stripped out:
- An assignment notebook is created by stripping out solution code cells (indication with cell tag `solution`)
...
For python files, the same approach is used as for markdown documents, but with `#` instead of `%`:
```py
# solution_start
...
# solution_end
...
Furthermore, assignment blocks can be added to the python files, which are stripped out in the solution version:
```py
# assignment_start
...
# assignment_end
...
## Git LFS storage
Images and binary or big files should be stored on our Git LFS repository. More information is available [on Git LFS here](./LFS.md).
